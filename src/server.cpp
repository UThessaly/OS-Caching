...
std::shared_ptr<Response> Server::HandleRequest(
  ...
  spdlog::warn("{}: Calculating Response ({}, {})", request->GetCacheId(), size,
               size % 10);
...
const std::chrono::milliseconds Server::GetTTL() const {
  ...
  return ttl + std::chrono::duration_cast<std::chrono::milliseconds>(
